...
public class JUnitPlugin {
	...
	public static void main(String[] args) {
		...
		for(Failure failure : graderResult.getFailures()) {
			...
			if(!checkAnnotationPresent(failure.getDescription(), autoAnno)) {
				continue;
			}
	...
	private static Properties loadProperties(String path) {
		String absolutePath = findPath(path);
		Properties groupProperties = new Properties();
		try(FileInputStream groupPropInputStream = new FileInputStream(absolutePath)) {
	...
	private static boolean checkAnnotationPresent(Description description, Annotation anno) {
		if(anno == null || description == null || !description.isTest()) {
			System.out.println(description + " does not have required annotation.");
			return false;
		}
		return true;
	}
	...
	private static String findPath(String path) {
		URL url = JUnitPlugin.class.getClassLoader().getResource(path);
		if(url == null) {
